...
PATH_SAVE_LOG = '/home/moojokeubuntu/KU/4_2/RealProject/webpattern/Input_3_min'
...
def sequence_gather(list_data):
    ...
    for i in range(1,len(list_link)):
        if link[-1]['domain'] == list_link[i]['domain']:
            if math.fabs(cal_time(link[-1]['time'])-cal_time(list_link[i]['time']))  > 180:
    ...
    compath = os.path.join(PATH_SAVE_LOG, 'input_user_cut_20to23.txt')
    count = 0
