...
def plot_per_build_5_day(sort,t):
    ...
    if count > 100 :
        ...
        #print(new_y)
        #plot(DATE, y,"k.",color = numpy.random.rand(3,1))
        plot(DATE, y,"k.")
        plot(DATE,new_y,"r",color = numpy.random.rand(3,1))
        xlim(datetime(2018, 4, 12, 0, 0, 0),datetime(2018, 4, 12, 23, 59, 59))
        ylim(0,300,10)
        xlabel("Time")
        ylabel("Number of Errors(Count)")
        # # grid(True)
        pylab.title('Large Group')
        compath = os.path.join(PATH_TO_SAVE1, 'Large_Group.png')
